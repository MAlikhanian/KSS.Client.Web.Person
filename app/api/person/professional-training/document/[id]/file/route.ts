...
// GET: Download a professional training document file by document ID (through orchestrator)
export async function GET(
  ...
  try {
    ...
    // Get professional training rows for the person, then flatten their nested documents
    const rows = await listSubByPerson(session.accessToken, 'ProfessionalTraining', personId);
    const docs = Array.isArray(rows)
      ? rows.flatMap((r: { professionalTrainingDocuments?: Array<{ id: string; storageInstanceId: number; fileName: string; contentType: string }> }) => r.professionalTrainingDocuments || [])
      : [];
    ...
    console.error('Error downloading professional training document file:', error);
