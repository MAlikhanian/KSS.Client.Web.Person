...
// GET: Download an education document file by document ID (through orchestrator)
export async function GET(
  ...
  try {
    ...
    // Get education rows for the person, then flatten their nested documents
    const rows = await listSubByPerson(session.accessToken, 'Education', personId);
    const docs = Array.isArray(rows)
      ? rows.flatMap((r: { educationDocuments?: Array<{ id: string; storageInstanceId: number; fileName: string; contentType: string }> }) => r.educationDocuments || [])
      : [];
    ...
    console.error('Error downloading education document file:', error);
